...
### 自动化 PR 检查（GitHub Actions）

当您对本仓发起或更新拉取请求时，仓库会通过 GitHub Actions 自动运行一组检查，帮助在早期捕捉常见问题并保持仓库一致性。检查包括（但不限于）：

- 针对 Markdown 的 lint 与链接检查
- Markdown 文件的拼写检查
- 仓库自定义的检查，例如：
    - 检测是否上传了 ebook 二进制文件（以确保二进制文件应放在单独的书籍仓库）
    - 验证 `books/` 下的作者/流派/专题目录命名规范
    - 验证 `docs/summaries/` 的文件名格式
    - 检测对 `library/` 文件的引用并检查这些文件是否存在于子仓（E-Library-Books-Repo）中

若检查失败，失败摘要会作为评论发布在 Pull Request 下，并且可以在 Actions 选项卡中查看详细信息。请修复报告的问题并将新提交推送到 PR，检查会自动重新运行。

来自 fork 的 PR 注意事项：

- `pull_request` 触发的 workflows 会对 fork 的 PR 运行，但 GitHub 可能会出于安全考虑阻止访问仓库 secrets。依赖 secrets 的步骤（例如使用 `GITHUB_TOKEN` 的 API 调用或需要写权限的评论创建）在 fork PR 上可能无法运行。如果您的 PR 来自 fork 并且遇到检查受限的情况，可考虑：
    - 将分支推送到您的 fork 并正常发起 PR，然后请求仓库维护者在需要时帮助运行需要 secret 的检查；或
    - 提交 issue 说明情况，方便维护者协助运行相关检查。
